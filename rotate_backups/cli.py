...
Supported options:
  ...
  -D, --depth=COUNT
    ...
    Consider files and directories to be backups if they are COUNT further
    levels below the contents of the given directory. The default is zero.
...
def main():
    ...
    try:
        options, arguments = getopt.getopt(sys.argv[1:], 'H:d:w:m:y:I:x:ri:c:r:uD:nvqh', [
            'hourly=', 'daily=', 'weekly=', 'monthly=', 'yearly=', 'include=',
            'exclude=', 'relaxed', 'ionice=', 'config=', 'use-sudo', 'depth=',
        ...
        for option, value in options:
            if option in ('-H', '--hourly'):
                ...
                if value.isdigit():
                  ...
                  raise ValueError("Invalid depth! (%s)" % value)
